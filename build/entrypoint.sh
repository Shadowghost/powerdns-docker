...
POSTGRES_DB=${POSTGRES_DB:-powerdns}
POSTGRES_PASSWORD=${POSTGRES_PASSWORD:-}

MYSQL_CLI="/usr/bin/mysql --host=${MYSQL_HOST} --user=${MYSQL_USER} --password=${MYSQL_PASSWORD} --port=${MYSQL_PORT} -r -N"
POSTGRES_CLI="/usr/bin/psql postgresql://${POSTGRES_USER}:${POSTGRES_PASSWORD}@${POSTGRES_HOST}:${POSTGRES_PORT}"
...
if ${PDNS_AUTOCONFIG} ; then
  ...
  case ${PDNS_BACKEND} in
    mysql)
      ...
      sed -i -E "s/(^gmysql-dbname=)(.*)/\1${MYSQL_DB}/g" ${PDNS_CONFIG_FILE};
      ...
      ;;
    pgsql)
      echo "PostgreSQL backend, configuring...";

      grep -q "^launch=" ${PDNS_CONFIG_FILE} || echo "launch=" | tee --append ${PDNS_CONFIG_FILE} > /dev/null;
      sed -i -E "s/(^launch=)(.*)/\1g${PDNS_BACKEND}/g" ${PDNS_CONFIG_FILE};

      grep -q "^gpgsql-host=" ${PDNS_CONFIG_FILE} || echo "gpgsql-host=" | tee --append ${PDNS_CONFIG_FILE} > /dev/null;
      sed -i -E "s/(^gpgsql-host=)(.*)/\1${POSTGRES_HOST}/g" ${PDNS_CONFIG_FILE};
      grep -q "^gpgsql-port=" ${PDNS_CONFIG_FILE} || echo "gpgsql-port=" | tee --append ${PDNS_CONFIG_FILE} > /dev/null;
      sed -i -E "s/(^gpgsql-port=)(.*)/\1${POSTGRES_PORT}/g" ${PDNS_CONFIG_FILE};
      grep -q "^gpgsql-user=" ${PDNS_CONFIG_FILE} || echo "gpgsql-user=" | tee --append ${PDNS_CONFIG_FILE} > /dev/null;
      sed -i -E "s/(^gpgsql-user=)(.*)/\1${POSTGRES_USER}/g" ${PDNS_CONFIG_FILE};
      grep -q "^gpgsql-dbname=" ${PDNS_CONFIG_FILE} || echo "gpgsql-dbname=" | tee --append ${PDNS_CONFIG_FILE} > /dev/null;
      sed -i -E "s/(^gpgsql-dbname=)(.*)/\1${POSTGRES_DB}/g" ${PDNS_CONFIG_FILE};
      grep -q "^gpgsql-password=" ${PDNS_CONFIG_FILE} || echo "gpgsql-password=" | tee --append ${PDNS_CONFIG_FILE} > /dev/null;
      sed -i -E "s/(^gpgsql-password=)(.*)/\1${POSTGRES_PASSWORD}/g" ${PDNS_CONFIG_FILE};
  ...
  case ${PDNS_BACKEND} in
    mysql)
      ...
      if [ $? -ne 0 ]; then
        ...
        if [ "$(echo "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = \"$MYSQL_DB\";" | $MYSQL_CLI)" -ne 0 ]; then
      ...
      ;;
    pgsql)
      echo "PostgreSQL backend, starting PowerDNS server...";
      /app/wait-for-it.sh -t 30 ${POSTGRES_HOST}:${POSTGRES_PORT}
      if [ $? -ne 0 ]; then
        echo "Error by connecting PostgreSQL database, could not initialize check."
        return 1 # error
      else
        echo "Check database..."
        TEST=$($POSTGRES_CLI -t -c "SELECT COUNT(*) FROM information_schema.tables WHERE table_catalog = '${POSTGRES_DB}';")
        echo "$TEST"
        if [ "$($POSTGRES_CLI -t -c "SELECT COUNT(*) FROM information_schema.tables WHERE table_catalog = '${POSTGRES_DB}';")" -ne 194 ]; then
          echo "Check successful, database exists..."
          return 0
        else
          appInit
  ...
  then
    ...
    case ${PDNS_BACKEND} in
      ...
      pgsql)
        ...
        /app/wait-for-it.sh -t 30 ${POSTGRES_HOST}:${POSTGRES_PORT}
        if [ $? -ne 0 ]; then
          echo "Error by connecting PostgreSQL database, could not start PowerDNS."
          return 1
        else
          echo "start PowerDNS..."
  ...
  case ${PDNS_BACKEND} in
    mysql)
      ...
      if [ $? -ne 0 ]; then
        ...
        ${MYSQL_CLI} ${MYSQL_DB} < /usr/share/pdns-backend-mysql/schema/schema.mysql.sql
        return $?
      fi

      ;;
    pgsql)
      echo "PostgreSQL backend, initializing...";
      /app/wait-for-it.sh -t 30 ${POSTGRES_HOST}:${POSTGRES_PORT}
      if [ $? -ne 0 ]; then
        echo "Error by connecting PostgreSQL database, could not initialize."
        return 1
      else
        ${POSTGRES_CLI}/${POSTGRES_DB} -f /usr/share/pdns-backend-pgsql/schema/schema.pgsql.sql
        return $?
      fi
